...
const Languages = () => {
  ...
  async function fetchCountryData() {
    try {
      ...
      const sortedCountries: CountriesType[] = response.data.sort(
        (a: CountriesType, b: CountriesType) => {
          return Object.keys(a.languages)[0] > Object.keys(b.languages)[0]
            ? 1
            : -1;
        }
      );
          ...
          {countries.map((item, idx) => {
            if (item.languages && Object.keys(item.languages).length > 0) {
              return (
                <MenuItem
                  key={idx}
                  value={item.languages && Object.keys(item.languages)[0]}
                >
